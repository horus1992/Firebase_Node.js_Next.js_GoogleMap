...
const UpdateUserAvatar = async (file, token) => {
  ...
  console.log("exist", exist)
  if (!file) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'File is not exist');
  }
  var user;
  if (exist) {
...
const getAllUsers = async (token) => {
  ...
  const me = await User.findOne(new firefose.Query().where('phoneNumber', '==', userInfo.phone_number))
  const others = await User.find(new firefose.Query().where('phoneNumber', '!=', userInfo.phone_number))
  return { me, others }
}
