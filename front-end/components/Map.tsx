...
const Map: NextPage<Props> = ({ canSet, canShowOthers, places, setPlaces, users }) => {
  ...
  const getUserCount = (users) => {
    if(!users){
      ...
      if(users.me){
        return users.others.length + 1
      }
      return users.others.length
    }
  }
  ...
  const mapClicked = async (info: any) => {
  ...
  return (
    <>
      <div className={styles.Map}>
        <div className={styles.Topbtn}>
          <div>
            <div className={styles.floatBtn}>
              ...
              <span>ユーザー数 :</span>
              <span>{getUserCount(users)}人</span>
            </div>
          </div>
        </div>
        {/* <PlacesAutocomplete
        ...
        <GoogleMap
          onClick={(e: any) => canSet && mapClicked(e.latLng.toJSON())}
          options={mapOptions}
          zoom={14}
          center={mapCenter}
